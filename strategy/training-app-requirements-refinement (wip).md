...
People already train with LLMs. They paste in their stats, describe their week, ask what they should do next. The advice is often good. But the experience is fragmented, manual, and context-free — the LLM never actually sees the data. It sees whatever the user remembered to type.

This app is the interface that closes that gap. It connects the data sources — Strava, Withings, power metrics — and gives the LLM something real to reason about. The user does not have to brief their coach before every session. The coach already knows. The app is not a training log with a chatbot bolted on. It is the interface *for* your LLM: a structured layer that turns raw athletic data into context an AI can act on.

The competitive insight is not "better AI coaching" — it is that the AI coaching already exists and people are already using it, badly. The opportunity is to make it good by solving the interface problem.

## App Structure — The Four Layers

The app is built around four things the user brings, in order:

1. **Their data** — fitness level, training history, current numbers. Sourced automatically from Strava and Withings. The user does not type this; the app reads it.
2. **Their goal** — what they are training toward and by when. Captured in onboarding. One clear target with a date.
3. **Their constraints** — other sports they do, life context, how many days they can train, when they prefer to train, how they relate to structure. Captured in onboarding, inferred from Strava where possible.
4. **Their plan** — a week-by-week path from where they are to the goal, generated by the LLM from the three inputs above. Displayed in the app, updated automatically as new data comes in.

This sequence is the structural spine of the product. Every screen, every feature, and every coaching interaction should be traceable back to one of these four layers. If something does not fit, it probably does not belong.

---

## The Core Product Loop

This is how people already use LLMs for training, and what the app needs to do better:

1. **They provide their data** — fitness level, recent training, current numbers. The app has this via Strava and Withings.
2. **They provide their goal** — what they are training toward and by when. The app captures this in onboarding.
3. **They provide their constraints** — other sports they do, life context, preferences, how many days they can realistically train. The app captures this in onboarding.
4. **They get a plan** — a structured week-by-week path from where they are to where they want to be.

Then the conversation usually dies. The plan sits in a chat thread. When something changes — a missed week, a new race on the calendar, a dip in motivation — they have to go back, re-brief the LLM, and ask for an update.

The app replaces that cycle. The plan lives in the interface. Progress is tracked automatically as new Strava activities come in. The LLM runs in the background, keeping the plan current without the user having to prompt it. And when the user needs it — to ask a question, understand a session, or adjust direction — it is already fully briefed and ready.

**The data structure the LLM needs to produce a good plan:**

| Input | Source |
|---|---|
| Current fitness (FTP, W/kg, weight) | Strava + Withings |
| Training history (last 8 weeks) | Strava |
| Goal (type, target, date) | Onboarding |
| Days per week available | Onboarding (inferred from Strava) |
| Preferred training times | Onboarding (inferred from Strava) |
| Life context | Onboarding |
| Other sports / cross-training | Strava activity history |
| Structure preference | Onboarding |

This is the full context package. Everything the user would normally have to type into a chat window, the app assembles automatically and keeps up to date.

---

## Bring Your Own LLM (BYOK)

If the app is an interface for your LLM, a natural extension is letting users connect their own LLM rather than relying on one the app provides. The user brings their API key — OpenAI, Anthropic, whatever they already pay for — and the app uses it to run analysis. The app's role becomes purely the data layer and the interface. No AI costs on the app's side, ever.

The economics are compelling: the user is already paying for ChatGPT Plus or Claude Pro. The app makes that subscription actually useful for training. The value proposition is clean — the app does not sell AI, it sells structured athletic context that makes any LLM dramatically better.

**What BYOK is not.** The tempting framing of "my LLM speaking with their LLM" implies something more agentic: the app's own model orchestrating a conversation with the user's model. That adds significant complexity for unclear gain. If the user brings their key, there is no need for a second LLM — you call their API directly with the structured data you have. The cleaner model: the app is the interface and the data; the user's LLM is the reasoning engine.

**The complication.** Most people who use ChatGPT do not have API keys. ChatGPT Plus is not the same as an OpenAI API key. The audience willing to bring their own key is smaller than the audience who already prompts LLMs about their training. BYOK makes the product harder to use for most people but philosophically purer.

**The pragmatic path.** Start with a hosted LLM to keep onboarding frictionless. Design the data layer so BYOK can be layered on later for users who want it. The two are not mutually exclusive — BYOK becomes a power-user option, not a requirement.

---

## Product implications

**The briefing problem is the core UX problem.** Every time a user opens ChatGPT to ask about their training, they spend the first few messages just bringing the LLM up to speed. If the app eliminates that entirely — the LLM already knows their FTP, last three weeks, goal, and current load — the quality of the output goes up and the friction of using it goes to near zero.

**Onboarding reframed.** It is not "set up your profile so we can coach you." It is "connect your data so your LLM actually knows who you are." That framing is more honest and more motivating for this audience.

**The Claude integration is the product, not a feature.** Future screens should lean into that: not a chatbot tab bolted on, but an always-present layer that has read everything and can speak to any of it.
